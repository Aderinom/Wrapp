...
//! 2. `DiInitiator` - which executes all factories, and handles injection requests during their
//!    initialization. 2.1. Injection Handles - which are valid during the `AppInit` Phase, and
//!    allow a component to request its dependencies.
...
//! Some Factory will not require a dependency (or just requires already existing instances), and as
//! such will complete After Completion, `AppInitiator` gets the result of the completed task.
