...
impl DiBuilder {
    ...
    /// These instances are stored in the DI container and can be retrieved by their type during container initialization.
    ///
    #[must_use]
    ...
    /// 
    /// [Factories](InstanceFactory) are responsible for creating instances of a specific type during initialization of the DI container. 
    /// They can have dependencies on other types, which will be resolved and injected by the DI container during [building](DiBuilder::build).
    #[must_use]
    ...
    /// - Fails if there are conflicts in the dependency graph, e.g. circular dependencies or missing required types.
    /// - Fails if two factories produce the same type, which would create ambiguity in the dependency graph.
